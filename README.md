...
FANDEX 诞生的初衷很简单：**帮助零基础学生迈出学习的第一步**。

当学习者在 FANDEX-web 上完成概念学习后，进入自主实践阶段时，经常会遇到以下场景：
...
FANDEX-App 就是为这些场景而生的。它不包含教学讲解，只提供**即查即用的语法参考**——语法签名、代码示例、使用公式。所有文档严格遵守统一的格式模板，确保查阅体验一致。

**核心理念：**

- 实践导向：为已具备基础的学习者提供实践中的语法速查
- 公式化呈现：每个语法点提供语法签名（公式）+ 代码示例，而非冗长讲解
- 完全离线：无需网络，随时随地查阅
...
作为学习者在移动场景下的离线伴侣，FANDEX-App 通过 Kotlin + Jetpack Compose 原生渲染，实现完全离线、即开即查的体验。无论是在通勤途中、午休间隙，还是在没有网络的环境中，都能随时查阅语法。
...
本仓库所有内容完全开源，任何个人或机构均可自由获取、使用、修改和分发，包括但不限于学习、研究、修改、分发及商业用途，无需获得授权。

### 免责声明

- 本仓库所有内容均由人工与人工智能（AI）共同编写、搜集、整理与编排，受限于编撰方式与知识更新速度，可能存在遗漏、过时或错误之处，使用者应结合官方文档与权威资料进行独立验证与核实
- 因使用本仓库内容所产生的一切后果，由使用者自行承担，本仓库作者及维护者不对使用后果承担任何形式的责任
