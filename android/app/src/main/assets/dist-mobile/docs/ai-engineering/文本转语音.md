...
**类型:** 构建
**语言:** Python
**前置条件:** Phase 6 · 02（频谱图与Mel），Phase 5 · 09（Seq2Seq），Phase 7 · 05（完整Transformer）
**时间:** ~75 分钟
...
- **采样率不匹配。** Kokoro输出24 kHz；你的下游流水线期望16 kHz → 重采样或产生混叠。
...
1. **简单。** 运行 `code/main.py`。从玩具词表构建音素字典，估算每个音素的时长，并打印假的"mel"调度。
2. **中等。** 安装Kokoro，用 `af_bella` 和 `am_adam` 声音合成同一句话。比较音频时长和主观质量。
3. **困难。** 录制5秒自己的参考片段。使用F5-TTS克隆。报告参考和克隆输出之间的SECS。
...
- [Kong, Kim, Bae (2020). HiFi-GAN](https://arxiv.org/abs/2010.05646) — 2026年仍在使用的声码器。
- [Kokoro-82M on HuggingFace](https://huggingface.co/hexgrad/Kokoro-82M) — 2024 CPU友好的英语TTS。
